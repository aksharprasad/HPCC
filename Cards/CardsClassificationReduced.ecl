IMPORT Cards.CardsSplit2Reduced as C;
IMPORT LearningTrees as LT;
IMPORT ML_Core;
IMPORT ML_Core.Discretize;

EXPORT CardsClassificationReduced := MODULE

EXPORT classify(INTEGER s, INTEGER e) := FUNCTION

xy1 := C.getTrainData(s,e);
x1 := xy1(number < 45);
ynd1 := PROJECT(xy1(number = 45), TRANSFORM(RECORDOF(LEFT), SELF.number := 1, SELF := LEFT)); 
y1 := Discretize.ByRounding(ynd1);

cf := LT.ClassificationForest(numTrees := 32, maxDepth := 32);
